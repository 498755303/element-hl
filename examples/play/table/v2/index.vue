<template>
  <div class="adesk-table">
    ...
    <el-table :ref="fixTableRef"
              ...
              :header-cell-style="headerCellStyleHandle"
        ...
        <el-table-column v-else-if="item.renderType == 'switch'"
          ...
          <div slot-scope="props"
            ...
            <el-switch v-model="props.row[item.prop]"
                       :disabled="item.switchDisabledKey ? props.row[item.switchDisabledKey] : false"
...
export default {
  ...
  methods: {
    ...
    headerCellStyleHandle({ row, column, rowIndex, columnIndex }) {
      if (column.property === this.curThead.prop) {
        column.order = this.curThead.order;
      }
      // console.log(this.curThead, column.property, column.order)
    ...
    // 滚动表头
    scrollFixTable(left) {
      this.fixTableBodyWrapper && (this.fixTableBodyWrapper.scrollLeft = left);
      this.fixTableHeaderWrapper && (this.fixTableHeaderWrapper.scrollLeft = left);
    },
    // 滚动表体
    scrollTable(left) {
